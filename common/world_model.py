...
class WorldModel(nn.Module):
	...
	def next(self, z, a, task):
		...
		Action is initially encoded through MLP, later as a rotation on the latent space 
		"""
		hang = self.s.hang
		...
		if isinstance(a, torch.Tensor):
			...
			with torch.no_grad():
				if isinstance(a, defs.Action):
					return unhang(torch.matmul(a.rotation, hang(z)))
				else:
					for i in range(split):
						for j in range(self.cfg.num_pi_trajs):
							z[i, j, :] = unhang(torch.matmul(a[i, j].rotation, hang(z[i, j, :])))
					return z
	...
	def reward(self, z, a, task):
		...
		if self.cfg.multitask:
			z = self.task_emb(z, task)
		if isinstance(a, defs.Action):
			z = torch.cat([z, a.embedding], dim=-1)
		else:
			z = torch.cat([z, a], dim=-1)
