		...
		for task_idx in tqdm(range(len(self.cfg.tasks)), desc='Evaluating'):
			...
			for _ in range(self.cfg.eval_episodes):
				...
				while not done:
					...
					if isinstance(action, defs.Action):
						act = action.embedding
						z = obs
						obs, reward, done, info = self.env.step(act)
						action.update_buffer(np.array([z, obs]))	
					else:
	...
	def train(self):
		...
		for i in range(self.cfg.steps):
			...
			if self.think_every:
				...
				if i == int(self.pretrain_frac*self.cfg.steps):
					self.agent.think()
			if _cfg.match_actions and self.agent.thought:
				self.agent.match_actions(self.cfg.pretrained_model_path)
				_cfg.match_actions = False
